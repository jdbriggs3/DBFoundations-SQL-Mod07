...
-- 2025-08-22,DBriggs,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_DBriggs')
	 Begin 
	  Alter Database [Assignment07DB_DBriggs] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_DBriggs;
	 End
	Create Database Assignment07DB_DBriggs;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_DBriggs;
...
SELECT TOP 1000000
	vC.CategoryName,
	FORMAT(vI.InventoryDate, 'MMMM, yyyy') AS [InventoryDate],		--Function FORMAT
	Sum(vI.Count) AS [InventoryCountByCategory]						--Function Sum
...
SELECT TOP 100000
	...
	vPI.InventoryDate, 
	vPI.InventoryCount,
	ISNull(Lag(vPI.InventoryCount)
		OVER(PARTITION BY vPI.ProductName
		ORDER BY CAST('01 '+ vPI.InventoryDate AS DATE)), 0) AS PreviousMonthCount
FROM vProductInventories AS vPI
ORDER BY vPI.ProductName, CAST('01 ' + InventoryDate AS DATE);
GO
...
SELECT TOP 1000000
    ...
    vPMC.InventoryDate,        
    vPMC.InventoryCount,
    vPMC.PreviousMonthCount,
    CASE
        WHEN vPMC.InventoryCount > vPMC.PreviousMonthCount THEN 1
        WHEN vPMC.InventoryCount = vPMC.PreviousMonthCount THEN 0
        WHEN vPMC.InventoryCount < vPMC.PreviousMonthCount THEN -1
        ELSE NULL
